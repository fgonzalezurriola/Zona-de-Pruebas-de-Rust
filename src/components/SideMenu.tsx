...
const SideMenu: React.FC<{ onSelectProblem: (problem: any) => void }> = ({ onSelectProblem }) => {
  ...
  return (
    <div className={`bg-gray-800 text-white transition-all duration-300 ${isExpanded ? 'w-64' : 'w-16'} overflow-y-auto`}>
      <button
      ...
      <nav>
        {problems.map((problem) => (
          <button
            key={problem.id}
            className="w-full text-left p-4 hover:bg-gray-700 transition-colors duration-200"
            onClick={() => onSelectProblem(problem)}
          >
            {isExpanded ? `Problem ${problem.id}: ${problem.title}` : problem.id}
          </button>
        ))}
      </nav>
    </div>
